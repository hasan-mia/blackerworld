    ...
    <div class="container-fluid">
        <div class="header my-3">
            <h1 class="header-title">
                Manage All Why Chose US
            </h1>
        </div>
        <div class="add-title">
            <a href="{{route('create.about')}}">Add Post</a>
        </div>
        {{-- end title --}}
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <table id="datatables-column-search-text-inputs" class="table table-striped" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($whychoseuses as $key=>$whychoseus)
                                <tr>
                                    <td>
                                        {{ $key + 1 }}. {{$whychoseus->title}}
                                    </td>
                                    <td>{{ ($whychoseus->created_at)->format('d/m/Y') }}</td>
                                    <td>
                                        <ul class="action">
                                            <li><a href="{{route('edit.whychoseus', $whychoseus->id)}}"
                                                    class="btn btn-success btn-sm">
                                                    <i class="fa fa-edit"></i></a> </li>
                                            <li>
                                                <form action="{!! route('delete.whychoseus', $whychoseus->id) !!}"
                                                    method="post">
                                                    @csrf
